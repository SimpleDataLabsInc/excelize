...
// ColumnsWithCorrespondingStyleIDs return the current row's column values along with their style IDs.
// This fetches the worksheet data as a stream, returns each cell in a row as is, and will not skip
// empty rows in the tail of the worksheet.
...
// additionally tracking per-cell style IDs.
...
// rowXMLHandlerV2 parse the row XML element of the worksheet, capturing each
// cell's style ID alongside its value.
